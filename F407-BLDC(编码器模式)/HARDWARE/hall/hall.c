...
u32 TIM5CH1_CAPTURE_VAL_1;	//输入捕获值(TIM2/TIM5为)32位
u32 TIM5CH1_CAPTURE_VAL_2;
u32 capture = 0;						//捕获差值
//定时器5中断服务函数
void TIM5_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM5,TIM_IT_CC1) != RESET)					//捕获1发生捕获
	{
		...
		if(TIM5CH1_CAPTURE_STA == 0)												//捕获第一个下降沿
		{
			TIM5CH1_CAPTURE_STA = 1;													//标记捕获一次下降沿
			TIM5CH1_CAPTURE_VAL_1 = TIM_GetCapture1(TIM5);		//获取当前捕获值1
		}
		else if(TIM5CH1_CAPTURE_STA == 1)
		{
			TIM5CH1_CAPTURE_STA = 0;
			TIM5CH1_CAPTURE_VAL_2 = TIM_GetCapture1(TIM5);		//获取当前捕获值2
			if(TIM5CH1_CAPTURE_VAL_2 > TIM5CH1_CAPTURE_VAL_1)
				capture = TIM5CH1_CAPTURE_VAL_2 - TIM5CH1_CAPTURE_VAL_1;
			else if(TIM5CH1_CAPTURE_VAL_2 < TIM5CH1_CAPTURE_VAL_1)
				capture = ((0XFFFFFFFF - TIM5CH1_CAPTURE_VAL_1) + TIM5CH1_CAPTURE_VAL_2);
			else capture = 0;
//			fq = 1000000 / capture;				//频率计算
		}
	}
}
//单位时间读取轮毂电机速度
