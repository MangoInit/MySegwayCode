...
#define	AT24CXX_DELAY_MS(n)		HAL_Delay(n)		/*	定义 ms 延时函数	*/
...
void AT24CXX_WriteByte(uint8_t Reg, uint8_t Value)
{
  I2Cx_Write(AT24CXX_ADDRESS, Reg, AT24CXX_MEMADD_SIZE, Value);
	...
	AT24CXX_DELAY_MS(5);		/*	延时是必须的，否则通信失败	*/
}
...
{
  return (I2Cx_ReadMultiple(AT24CXX_ADDRESS, MemAddress, AT24CXX_MEMADD_SIZE, pBuffer, BufferSize));
}
...
uint8_t AT24CXX_Check(void)
{
	...
	if(temp==0XBB)return 0;		   
	else//排除第一次初始化的情况
	{
		AT24CXX_WriteByte(AT24CXX_TYPE,0XBB);
