...
void TIM5_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM5,TIM_IT_CC2) != RESET)						//捕获1发生捕获
	{
		TIM_ClearITPendingBit(TIM5, TIM_IT_CC2 | TIM_IT_Update);		//清除中断标志位
		if(TIM5CH2_CAPTURE_STA == 0)									//捕获第一个上升沿
		{
			TIM_OC2PolarityConfig(TIM5,TIM_ICPolarity_Falling);			//CC2P=1 设置为下降沿捕获
			TIM5CH2_CAPTURE_STA = 1;									//标记捕获一次下降沿
			TIM5CH2_CAPTURE_VAL++;
		}
		else if(TIM5CH2_CAPTURE_STA == 1)
		{
			TIM_OC2PolarityConfig(TIM5,TIM_ICPolarity_Rising);			//CC2P=1 设置为下降沿捕获
			TIM5CH2_CAPTURE_STA = 0;									//标记捕获一次下降沿
			TIM5CH2_CAPTURE_VAL++;
		}
	}
}
