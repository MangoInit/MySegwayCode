...
{
	TIM3->SR&=~(1<<0);	//清除中断标志位 
	TIM3->ARR=0XFFFF;	//将重装载值设置到最大
	TIM3->CNT=0;		//清空定时器的CNT
...
{
	if(TIM3->SR&0X0001)//在运行期间,产生了定时器溢出
	{
		usmart_dev.runtime+=0XFFFF;
	}
	usmart_dev.runtime+=TIM3->CNT;
...
//	RCC->APB1ENR|=1<<2;   	//TIM4 时钟使能   
	...
	TIM3->CR1|=0x01;    	//使能定时器4
  	MY_NVIC_Init(3,3,TIM3_IRQn,2);//抢占3，子优先级3，组2(组2中优先级最低的)									 
}
#endif
////////////////////////////////////////////////////////////////////////////////////////
//初始化串口控制器
...
{
#if USMART_ENTIMX_SCAN==1
	Timer3_Init(1000,(u32)sysclk*100-1);//分频,时钟为10K ,100ms中断一次,注意,计数频率必须为10Khz,以和runtime单位(0.1ms)同步.
#endif
